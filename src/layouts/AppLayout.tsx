...
flex justify-between items-center px-4 z-50 border-[0.50px]"
    ...
    blur-[0.40px]"
          ...
          {open && (
            <div className="fixed right-3 top-1 gap-2 w-[130px] h-[110px] flex flex-col-reverse bg-neutral-700/60 backdrop-blur-3xl shadow-lg rounded-md rounded-tr-[20%] p-2">
              ...
              >
                <img
                  src={powerIcon}
                  className="w-4 h-4 opacity-80 [text-shadow:0_1px_12px_rgba(0,0,0,1)]"
                />
                Logout
              </button>
              <button
      ...
      <main
        className="pt-16 pb-16 h-screen overflow-y-auto p-4 bg-custom-100/60"
        onClick={() => {
          setOpen(false);
        }}
      >
          ...
          <div className="relative w-80 h-12 flex items-center justify-center">
            {tabs.map((tab, index) => {
              ...
              if (offset === 0) {
                ...
                style =
                  "text-indigo-400 [text-shadow:0_1px_12px_rgba(0,0,0,1)] text-sm font-bold ";
              } else if (offset === 1 || offset === -2) {
                transform = "translate-x-[105px] translate-y-[10px]";
                style =
                  "text-indigo-400 [text-shadow:0_1px_12px_rgba(0,0,0,1)] text-sm font-bold";
